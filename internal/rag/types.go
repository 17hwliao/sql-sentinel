...
var (
	...
	ErrCitationMismatch = errors.New("rag: answer citation is not present in retrieved context")
)
...
type ContextItem struct {
	...
	Text      string   `json:"text"`
	Untrusted bool     `json:"untrusted"`
	// Content is the exact chunk payload, kept out of JSON so a caller can
	// hash-bind it to a grounded-answer request without removing the untrusted
	// delimiters used for local display.
	Content string `json:"-"`
}
