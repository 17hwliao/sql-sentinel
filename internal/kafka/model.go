...
var ErrReplayNotEligible = errors.New("kafka workflow job is not dead-lettered")
...
// HistoryEntry is intentionally metadata-only. Failure details remain inside
// the control-plane database and are never exposed by the control API.
...
type Store interface {
	...
	ListDeadLetters(context.Context, int) ([]Job, error)
	History(context.Context, string, int) ([]HistoryEntry, error)
	ReplayDeadLetter(context.Context, string, time.Time) (Job, bool, error)
	ClaimOutbox(context.Context, int, time.Time, string, time.Duration) ([]OutboxRecord, error)
	MarkPublished(context.Context, string, string, time.Time, error) error
}

var ErrOutboxLeaseLost = errors.New("kafka workflow outbox lease lost")
...
func (c Config) withDefaults() Config {
	...
	if c.LeaseRenewEvery <= 0 || c.LeaseRenewEvery >= c.Lease {
		c.LeaseRenewEvery = c.Lease / 3
	}
	if c.RunTimeout <= 0 {
